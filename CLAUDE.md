...
3. Look at the pyproject.toml (if a python project)
3. Install dependencies if necesary:  `uv sync --active`
...
- **File Location**: Never put stray Python, test, text, or markdown files in the project root. Place them in organized locations:
  - Python modules in `src/project_name/`
  - Tests in `tests/`
  - Documentation in `src/project_name/docs/`
  - Examples in `src/project_name/examples/`
